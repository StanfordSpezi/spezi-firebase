...
import { Document } from '../../src/storage/deviceStorage.js';
...
describe('FirebaseNotificationService', () => {
  ...
  describe('unregisterDevice', () => {
    it('should call deviceStorage.removeDevice with correct parameters', async () => {
      ...
      expect(mockDeviceStorage.removeDevice.firstCall.args[2]).to.equal(platform);
    });
  });
  
  describe('sendNotification', () => {
    let devices: Document<Device>[];
    ...
    it('should create token messages for each device', async () => {
      ...
      const androidMessage = tokenMessages.find((m: any) => m.token === 'android-token');
    ...
    it('should handle failed notifications and remove invalid tokens', async () => {
      ...
      expect(mockDeviceStorage.removeInvalidToken.firstCall.args[0]).to.equal('android-token');
    });
  });
